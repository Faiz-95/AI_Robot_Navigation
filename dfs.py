...
def visit_position(search, robot_map):
    if ((len(search.not_visited)) > 0):
        search.current_position = search.not_visited[0] # Since dfs follows LIFO, we always assign current position to the first position that is not visited
        # Note that the first position in not_visited is the last position added to it
...
def find_valid_adjacents(search, robot_map):
    ...
    # Robot follows the opposite of UP-LEFT-DOWN-RIGHT priority in directions in reverse order
...
def check_adjacent_value(new_direction, row_num, col_num, search,robot_map):
    ...
    elif ((robot_map.grid[row_num][col_num] == " ") or (robot_map.grid[row_num][col_num] == "X")):
...
def add_adjacents_to_not_visited(search, robot_map):
    for adj in search.adjacents:
        ...
        search.not_visited.insert(0,adj) # Inserts the position to the beginning of the not_visited list
