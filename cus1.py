...
# Creating Custom 1 Search object
class Cus1:
    def __init__(self):
        self.nodes = 0
    ...
    for row in robot_map.grid:
        for column in row:
            if column == "X": # Check to see which nodes have not been searched yet 
                x_nodes += 1 # Add the not_visited nodes to the total
...
def find_start_and_end(search, robot_map):
    ...
    for i, row in enumerate(robot_map.grid):
        for j, cell in enumerate(row):
            if cell == "S":
                search.visited.append([search.directions, [i, j]]) # Considering start position to be a visited position
                found_start = True
            elif cell == "F":
...
def visit_position(search, robot_map):
    if ((len(search.not_visited)) > 0):
        random.shuffle(search.not_visited) # Shuffle the not_visited list to randomize the new position
        ...
        search.visited.append(search.not_visited[0]) # Assign the new position of current_position to visited list
