...
# Creating Custom 2 Search object
class Cus2:
    def __init__(self):
        self.nodes = 0
        self.avalue = 0
...
def find_start_and_end(search, robot_map):
    ...
    for i, row in enumerate(robot_map.grid):
        for j, cell in enumerate(row):
            if cell == "S":
                search.avalue = manhattan_distance(search, [i,j])
                search.visited.append([[[0,search.avalue],search.directions],[i,j]]) # Considering start position to be a visited position
